...
export function LeadForm() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setServerMessage(
          result.message || "Не удалось отправить заявку. Проверьте поля.",
        );
        setStatus("error");
        return;
      }
  ...
  return (
    <form
      className="rounded-3xl border border-borderline bg-white p-6 shadow-sm sm:p-8"
    ...
    >
      ...
      <label className="mt-5 flex gap-3 text-sm leading-6 text-muted">
        ...
        />
        <span>
          Я согласен на обработку персональных данных и понимаю, что не нужно
          отправлять конфиденциальные файлы через форму.{" "}
          <Link
      ...
      <div className="mt-6 flex flex-wrap items-center gap-4">
        <button
          className="focus-ring rounded-full bg-primary px-6 py-3 font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-400"
