    ...
    <footer className="border-t border-borderline bg-white">
      <div className="container-page grid gap-10 py-12 md:grid-cols-[1.2fr_0.8fr_0.8fr]">
        <div>
          ...
          <p className="mt-4 max-w-md leading-7 text-muted">{siteConfig.description}</p>
          <p className="mt-4 text-sm text-slate-500">Материалы сайта не являются публичной офертой.</p>
        </div>
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-500">Разделы</h2>
          <ul className="mt-4 space-y-3">
            {siteConfig.footerLinks.map((link) => (
              <li key={link.href}>
                <Link className="focus-ring rounded-md text-muted transition hover:text-primary" href={link.href}>
                  {link.label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-muted">
            <li>
              <a className="focus-ring rounded-md transition hover:text-primary" href={"mailto:" + siteConfig.contacts.email}>
